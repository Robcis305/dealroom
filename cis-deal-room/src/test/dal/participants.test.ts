...
describe('validateShadowSide via inviteParticipant', () => {
  ...
  it('invites a view_only participant without shadow side (column removed in 0018)', async () => {
    vi.mocked(verifySession).mockResolvedValue(adminSession);
    mockSelectChain
      .mockResolvedValueOnce([{ id: 'user-1' }])  // user exists
      .mockResolvedValueOnce([]);                   // no existing participant
    mockInsertReturning.mockResolvedValueOnce([
      { id: 'p-view', userId: 'user-1', role: 'view_only', status: 'invited' },
    ]);
    ...
    // viewOnlyShadowSide column is retired — the insert values must not include it
    expect('viewOnlyShadowSide' in (participantInsertCall![0] as object)).toBe(false);
  });
});
