// Apply migration 0011_seed_playbook directly via the working driver.
// Seeds the 48 canonical playbook items into playbook_items.
// Idempotent: ON CONFLICT ("number") DO NOTHING — re-running is safe.
...
console.log('=== seed playbook_items ===');

// Single multi-value INSERT — ON CONFLICT ("number") DO NOTHING makes it idempotent.
await sql`
...
// Verify
console.log('\n=== verify ===');
const [{ count }] = await sql`SELECT count(*)::int AS count FROM playbook_items`;
console.log('playbook_items count:', count);

const [{ killer_count }] = await sql`SELECT count(*)::int AS killer_count FROM playbook_items WHERE deal_killer_group IS NOT NULL`;
console.log('deal_killer count:', killer_count);

const rows8 = await sql`SELECT name FROM playbook_items WHERE number = 8`;
const name8 = rows8[0]?.name;
console.log('item 8 name:', name8);

if (count !== 48 || killer_count !== 7) {
  console.error(`ERROR: expected 48 total / 7 deal-killers, got ${count} / ${killer_count}`);
  process.exit(1);
}
if (name8 !== '83(b) election filings with proof of mailing') {
  console.error(`ERROR: item 8 name mismatch: ${name8}`);
  process.exit(1);
}
